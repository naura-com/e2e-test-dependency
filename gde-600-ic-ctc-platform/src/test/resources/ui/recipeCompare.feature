...
功能: 工艺配方比较界面
  ...
  背景:
    假如存在"defaultEtch1Rcp Recipe":
      | name | rcpClass |
      | test | /Process |
    假如存在"defaultEtch1Rcp_step1_step2 Recipe":
      | name | rcpClass |
      | dtest | /DryClean |
  ...
  Rule: Compare
    ...
    场景: Choose PM A和Choose PM B类型不同
      假如Page以"SetPoint"方式比较如下Recipe:
        | condition         | A       | B        |
        | Choose PM         | PM2 GDE | PM2 GDE  |
        | Choose RecipeType | Process | DryClean |
        | Choose Recipe     | test    | dtest    |
      那么执行失败,弹出提示信息:
      """
  ...
  Rule: Page
    ...
    @etch1-clean-recipeCompare
    场景: Page为空,提示信息验证
      假如Page以""方式比较如下Recipe:
        | condition         | A       | B       |
        | Choose PM         | PM2 GDE | PM2 GDE |
        | Choose RecipeType | Process | Process |
        | Choose Recipe     | test    | test    |
      那么执行失败,弹出提示信息:
      """
