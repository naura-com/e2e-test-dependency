...
功能: Chamber
  ...
  Rule: Method
    ...
    场景: 正常Initialize验证
      ...
      那么当前页面显示如下:
      """
      getModuleStatus['Initialize']= Standby
      """
    ...
    场景: 正常Maintain验证
      ...
      那么当前页面显示如下:
      """
      getModuleStatus['Maintain']= MaintMode
      """
...
#      """
  ...
  Rule: Set Gas Flow
    ...
    场景大纲: 单个气体流量设置_流量为空:<GasFlow>
      ...
      例子:
        | GasFlow | message               |
        | Ar      | Error value in 1 row! |
        | O2      | Error value in 2 row! |
        | He      | Error value in 3 row! |
        | N2      | Error value in 4 row! |
        | SF6     | Error value in 5 row! |
        | CF4     | Error value in 6 row! |
        | Cl2     | Error value in 7 row! |
        | Bcl3    | Error value in 8 row! |
    ...
    场景大纲: 单个气体流量设置_阈值内:<title>
      当设置气体流量值如下:
        | <title> |
        | <value> |
      那么执行成功,"<title>"的Set Point显示正确:
      """
      = "<value>"
      """
      例子:
...
#      那么Run Service显示"do Vent"
  ...
  Rule: Pump
    ...
    场景大纲:设置泵<pumpName>状态为<setStatus>
      假如"PM2 GDE"处于"Maintain"模式
      假如设备""状态为"<initStatus>"
      当设置Pump"<pumpName>"为"<setStatus>"
      那么泵"<pumpName>"在服务窗口在服务窗口和界面显示为"<setStatus>"
      例子:
        | pumpName  | initStatus      | setStatus |
        | TurboPump | TurboPump_Start | Off       |
        | DryPump   | DryPump_Start   | Off       |
        | DryPump   | DryPump_Stop    | On        |
...
#      那么"TurboPump"显示为"On"
  ...
  Rule: 上位机设置阀门
    ...
    场景大纲:打开阀门<valve>
      假如"PM2 GDE"处于"Maintain"模式
      假如设备"<valveName>"状态为"CloseValve"
      当设置阀门"<valve>"为"Open"
      那么阀门"<valve>"在服务窗口和界面显示为"Open"
      例子:
        | valve          | valveName         |
        | TPN2           | TMPPurgeValve     |
        | ISO            | TMPIsolationValve |
        | fast           | FastRough         |
        | slow           | SlowRough         |
        | HePrimary      | HeValve1          |
        | HeStabilize    | HeValve3          |
        | HeForelineEVAC | HeValve4          |
        | HeSecondary    | HeValve2          |
        | Final          | FinalValve        |

    场景大纲:关闭阀门<valve>
      假如"PM2 GDE"处于"Maintain"模式
      假如设备"<valveName>"状态为"OpenValve"
      当设置阀门"<valve>"为"Close"
      那么阀门"<valve>"在服务窗口和界面显示为"Close"
      例子:
        | valve          | valveName         |
        | TPN2           | TMPPurgeValve     |
        | ISO            | TMPIsolationValve |
