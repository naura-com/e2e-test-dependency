...
import com.naura.test.ice.IComControlObject.*;
...
//        winForm.clickById("Process Recipe");
    ...
    private void startIceMockServer() {
        ...
        doReturn(new String[]{"LightTower_Red", "LightTower_Yellow", "LightTower_Green", "LightTower_Blue", "LightTower_Buzzer2"}).when(remoteAppEntrance).getNamespaceNodesUnder(eq("/Control/TMCExports/Datas"), any(Current.class));
        ...
        doAnswer(invocationOnMock -> {
            Etch1LowerSteps.remoteObjectType = RemoteObjectType.DATA;
            return new String[0];
        }).when(remoteAppEntrance).getNamespaceNodesUnder(eq("LightTower_Buzzer2"), any(Current.class));
    ...
    public static class RemoteRecipeUserI extends _RemoteRecipeUserDisp {
        ...
        public boolean getRcpClassInfo(String fullName, RemoteRcpClassInfoHolder clsInfo, Current __current) {
            RemoteDoubleParamInfo a = new RemoteDoubleParamInfo("ProcessTime", "", "0", RemoteDataType.DOUBLEDATA, new RemoteDoubleTypeInfo(0, 360000, "sec", 0.1));
            RemoteDoubleParamInfo[] params = new RemoteDoubleParamInfo[]{a};
            clsInfo.value = new RemoteRcpClassInfo(params, new String[]{"/Process/test0;1"});//newString-->>"/Process/test0;1"
            return true;
        }
