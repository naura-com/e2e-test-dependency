...
public class QuerySteps {
    ...
    public void queryResult显示查询结果(DataTable table) {
        mainPage.alarmPage().queryPage().createQueryResult(table);
    }
    ...
    public void 点击selectall(String type) {
        mainPage.alarmPage().queryPage().clickSelectall(type);
    }

    @那么("所有PM,TM被选中")
    public void 所有pmTM被选中() {
        mainPage.alarmPage().queryPage().verifyAllChamber();
    }

    @那么("所有报警级别被选中")
    public void 所有报警级别被选中() {
        mainPage.alarmPage().queryPage().verifySeverity();
    }
