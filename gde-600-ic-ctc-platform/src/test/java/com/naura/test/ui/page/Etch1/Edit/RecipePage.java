...
//        TODO: to add a method
    ...
    public void verifyStep(int first, int second) {
        ...
        for (Map<Object, Object> objectObjectMap : result) {
            assertThat(objectObjectMap.get(first)).isEqualTo(objectObjectMap.get(second));
...
//        lowerApi.recipe().storercpForEtch1justOneStep(valueList);
    ...
    public void hasSavedRcp(String pmName, String rcpClass, String rcpName, DataTable table) {
        createStepRcp(pmName, rcpClass, rcpName, table);
        clickAction("Save");
        clickOkOrCancel("Ok");
    }

    public void modifyPar(String rcp, int step, String parameter, String value) {
        selectRcp(rcp);
        modifyStepName(step, parameter, value, "double");
    }
    ...
    public void confirmSaveAs(String preRcpClass, String preRcp, String pmClass, String rcp) {
        selectRcpType(preRcpClass, false);
        selectRcp(preRcp);
        saveAsRcp(pmClass, rcp);
    }

    public void saveAs(String rcpClass, String rcp) {
        selectRcpType(rcpClass, false);
        selectRcp(rcp);
        clickAction("Save as");
    }
    ...
    public void clear() {
        clickOkOrCancel("Yes");
        clickOkOrCancel("OK");
        clickOkOrCancel("Cancel");//这一部分保存了改变并取消了新建
    }

    public void confimSaveAndCreate(String rcp) {
        clickOkOrCancel("Yes");
        clickOkOrCancel("OK");
        sendNewRcpName(rcp);
        clickOkOrCancel("Ok");
    ...
    public void noSaveAndCreate(String rcp) {
        clickOkOrCancel("No");
        sendNewRcpName(rcp);
        clickOkOrCancel("Ok");
    }

    public void deleteRcp(String rcpClass, String rcp) {
        selectRcpType(rcpClass, false);
        selectRcp(rcp);
        clickAction("Delete");
    }

    public void confirmDelete(String rcpClass, String rcp) {
        deleteRcp(rcpClass, rcp);
        clickOkOrCancel("Yes");
    }
    ...
    public void confirmExport(String rcpClass, String rcp) {
        selectRcpType(rcpClass, false);
        selectRcp(rcp);
        export("Full Steps", "", "");
    }

    public void confirmSaveAs(String rcpClass, String preRcp, String renameRcp) {
        selectRcpType(rcpClass, false);
        selectRcp(preRcp);
        rename(renameRcp);
    }

    public void selectAndsaveAs(String rcpClass, String preRcp) {
        selectRcpType(rcpClass, false);
        selectRcp(preRcp);
        clickAction("Rename");
    }

    public void exportSubStep(String rcpClass, String rcp, String startStep, String endStep) {
        selectRcpType(rcpClass, false);
        selectRcp(rcp);
        export("Sub Steps", startStep, endStep);
    }

    public void modifyAndSave(DataTable table) {
        modifyRcpParas(table);
        clickAction("Save");
        clickOkOrCancel("OK");
    }
}
