...
功能: manual process
  ...
  背景:
#    假如存在如下已保存配方:
#      | PMName  | RecipeClass | RecipeName | ProcessTime | WaferType | Pressure(mT) |
#      | PM2 GDE | Process     | test       | 2           | Si        | 1            |
  ...
  Rule: Run Process
    ...
    场景: Run Process未选择recipe提示信息验证
      假如RecipeList未选择recipe
      当选择执行"Run Process"并二次确认时
      那么执行失败,弹框提示为:
      """
...
#      假如存在"Processing"状态的"test2"
#      当第"1/2"步选择执行"Manual Endpoint"
#      那么Step Info列表信息为:
#      """
#      :{
#      StepId: "2/2"
