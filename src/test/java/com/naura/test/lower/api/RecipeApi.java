...
public class RecipeApi {
    ...
    public RecipeApi(RemoteRecipeUserPrx remoteRecipeUserPrx) {
    ...
    public Object invoke(String api, Table table) {
        switch (api) {
            ...
            case "registerRcpClass": {
                registerRcpClass((String) table.get(0).get("name"), ((List<Map<String, ?>>) table.get(0).get("params")).stream().map(RecipeApi::buildParam).toArray(RemoteParamInfo[]::new));
                return null;
            }
            case "storeRcpBody": {
    ...
    public void storercpForEtch1justOneStep(String[] strings, String rcp) {
        shouldTrue(stringHolder -> remoteRecipeUserPrx.storeRcpBody("/Process/" + rcp + ";1", rcpProForEtch1JustOneStep(strings), true, stringHolder));
    }
...
//        return storeRcpBody;
    ...
    public class Test {
        ...
        public void cleanRecipeClass() {
            stream(getAllRcpClasses()).filter(this::isSubClass).sorted(Comparator.comparing(String::length).reversed()).forEach(RecipeApi.this::removeRcpClass);
        }
