...
//    }
    ...
    public void 在的存在已保存配方(String pmName, String rcpClass, String rcpName, DataTable table) {
        mainPage.editPage().recipePage().createStepRcp(pmName, rcpClass, rcpName, table);
...
//        mainPage.editPage().recipePage().cleanAllCreateRcp();
    ...
    public void 执行recipe的操作(String operation) {
        switch (operation) {
            case "Save":
                mainPage.editPage().recipePage().enterAndSave();
                break;
            case "New":
                mainPage.editPage().recipePage().clickAction("New");
                break;
            case "Restore":
                mainPage.editPage().recipePage().enterAndRestore();
                break;
            default:
        }
    }
    ...
    public void 选择的另存为并且二次确认后(String preRcpClass, String preRcp, String pmClass, String rcp) {
        mainPage.editPage().recipePage().selectRcpType(preRcpClass, false);
        mainPage.editPage().recipePage().selectRcp(preRcp);
    ...
    public void 选择的另存为(String rcpClass, String rcp) {
        mainPage.editPage().recipePage().selectRcpType(rcpClass, false);
        mainPage.editPage().recipePage().selectRcp(rcp);
        mainPage.editPage().recipePage().clickAction("Save as");
    }
    ...
    public void 新建前弹出框提示信息(String vetifyInfo) {
        ...
        mainPage.editPage().recipePage().clickOkOrCancel("Yes");
        mainPage.editPage().recipePage().clickOkOrCancel("OK");
        mainPage.editPage().recipePage().clickOkOrCancel("Cancel");//这一部分保存了改变并取消了新建
    }
    ...
    public void 确认保存后再新建recipe(String rcp) {
        mainPage.editPage().recipePage().clickOkOrCancel("Yes");
        mainPage.editPage().recipePage().clickOkOrCancel("OK");
        mainPage.editPage().recipePage().sendNewRcpName(rcp);
        mainPage.editPage().recipePage().clickOkOrCancel("Ok");
    }

    @并且("确认不保存后再新建Recipe{string}")
    public void 确认不保存后再新建recipe(String rcp) {
        mainPage.editPage().recipePage().clickOkOrCancel("No");
        mainPage.editPage().recipePage().sendNewRcpName(rcp);
        mainPage.editPage().recipePage().clickOkOrCancel("Ok");
    }
    ...
    public void 删除的(String rcpClass, String rcp) {
        mainPage.editPage().recipePage().selectRcpType(rcpClass, false);
        mainPage.editPage().recipePage().selectRcp(rcp);
        mainPage.editPage().recipePage().clickAction("Delete");
    }
    ...
    public void 选择的重命名为并且二次确认后(String rcpClass, String preRcp, String renameRcp) {
        mainPage.editPage().recipePage().selectRcpType(rcpClass, false);
        mainPage.editPage().recipePage().selectRcp(preRcp);
        mainPage.editPage().recipePage().rename(renameRcp);
    }
    ...
    public void 选择的重命名(String rcpClass, String preRcp) {
        mainPage.editPage().recipePage().selectRcpType(rcpClass, false);
        mainPage.editPage().recipePage().selectRcp(preRcp);
        mainPage.editPage().recipePage().clickAction("Rename");
    }
    ...
    public void 导出的的步骤到步骤为文件(String rcpClass, String rcp, String startStep, String endStep, String exportFile) {
        mainPage.editPage().recipePage().selectRcpType(rcpClass, false);
        mainPage.editPage().recipePage().selectRcp(rcp);
        mainPage.editPage().recipePage().export("Sub Steps", startStep, endStep);
        mainPage.editPage().recipePage().exportRcpFile(virtualDir, exportFile);
    }
