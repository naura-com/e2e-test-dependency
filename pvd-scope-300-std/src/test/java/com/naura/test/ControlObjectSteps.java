...
//            TODO hardcode /IO/Ch1Exports/State now, should transformed from name
            mockLower.mockRemoteValueInfo("/IO/Ch1Exports/State", remoteStringValueInfo("Offline"));

//            TODO hardcode /IO/Ch1Exports/Solenoid_V21DI now, should transformed from name
            mockLower.mockRemoteValueInfo("/IO/Ch1Exports/Solenoid_V21DI", remoteIntValueInfo(status.equals("Open") ? 1 : 0));
        } else {
    ...
    public void io状态为(String verification) {
        await().timeout(5, TimeUnit.SECONDS).untilAsserted(() -> expect(lowerApi).should(verification));
    }
